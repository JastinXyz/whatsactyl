...
module.exports = {
    ...
    getNo: function getNo(msgy) {
       /*if(msg.key.remoteJid.endsWith('@g.us')) {
        ...
        return msgy.key.fromMe ? whats.user.id : msgy.key.participant ? msgy.key.participant : msgy.key.participant ? msgy.key.participant : msgy.key.remoteJid
    },
    shortlink: async function shortlink(url) {
//nothing
    }, 
    // ini api client (di dalam)
    client: {
...
const y = await axios({
        ...
        userAccount: async function userAccount(keyid) {
			...
			/*const y = await axios({
                ...
                maxRedirects: 5,
                headers: {
                'Authorization': 'Bearer '+ keyid,                    'Content-Type': 'application/json',
                    'Accept': 'Application/vnd.pterodactyl.v1+json',
                }                
            })
         return y.data*/
...
axios({
    ...
    serverSendCommand: async function serverSendCommand(serverid, keyid, message) {
const y = await axios({
    ...
    // ini  application (di dalam)
    application: {
        name: function name() {
            // do something
        },
        aname: function aname() {
            // do something
        }
    }
}