            ...
            if(args[0] == "create") {
                if(!args[1]) {
                    reply(`\`\`\`${prefix}backup create [server id]\`\`\``)
                 	...
                 	_c.createServerBackup(args[1]).then(async (x) => {
const urlbackup = await _c.getBackupDownload(args[1], x.uuid)
 
msg.key.remoteJid.endsWith('@g.us')? whats.sendMessage(getNo(msg).replace(":12", ""), { text: `*➥ Download: ${urlbackup.url}*` }, {quoted: msg}).then(reply(`*✅ | Created!*\n\n*➥ Download: sended in dm.*`)) : reply(`*✅ | Created!*\n\n*➥ Download: ${urlbackup.ur}*`)
                        //reply(`*✅ | Created!*\n\n*➥ Download: ${urlbackup.url}*`)
            		}).catch((e) => {
                ...
                if(!args[1]) {
                    ...
                    _c.deleteBackup(args[1], args[2]).then(() => {
                        reply(`*✅ | Deleted!*`)
                    }).catch((e) => {
                		error(`is the server id valid?\n\n*ERROR LOGS:*\n${e}`)
            		})
                ...
                if(msg.key.remoteJid.endsWith('@g.us')) {
                    ...
                    _c.listServerBackups(args[1]).then(async(x) => {
                      const n = await _c.getServerDetails(args[1])
                       let num = 0
let teks = `*〔 ${n.name} Backup List 〕*\n\n`
//let teks = `LIST BACKUP SERVER:\n\n`
for (let i of x) {
let y = i.attributes
num += 1
//teks += `Num: ${num}\nUUID: ${y.uuid}\n\n`
...
reply(teks)
        	...
        	error(`try whether your apikey client is valid or server is exists?. Or ask the developer about this problem...\n\n*ERROR LOGS:*\n${err}`)
    	}   
    }
};
